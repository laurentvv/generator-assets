...
* **Chanson complète avec paroles (validée le 2026-09-06)** : « La Symphonie du Silence » puis « Le Neuvième Fils » (univers Vent-Gris, paroles utilisateur) — 240-260 s, paroles FR via `--lyrics` + `--language fr` (param `langue` de `generer_musique_acestep`), balises de structure `[Intro]/[Verse]/[Chorus]/[Bridge]/[Outro]` respectées, xl-turbo, **~924 s de génération pour 240 s (RTF ~3,9)**. Nettoyage des paroles indispensable : retirer toute citation/annotation (sinon chantée). Script générique : `scripts/generer_chanson_acestep.py paroles.txt --duree 240`. Routes d'édition (repaint/cover/lego/extract) toujours non testées.
