...
* **🟡→✅ SA3 Medium ESSENCE init_audio (2026-09-08, verdict utilisateur : échelle 0,5 = « le plus proche ! »)** — suite du test texte (pop) : mode `init_audio` avec référence LLB (`llb_extrait_30s.wav`), prompt sobre identique, graine 42, 30 s, échelles 0,7 et 0,5 testées. **RTF 0,80** (24 s/génération). Verdict relatif : **0,5 > 0,7** (« le plus proche » de la référence) — le conditionnement audio contourne le biais pop du texte, SA3 Medium fait mieux que small (qualité rejetée en son temps). Fichiers : `output/test_sa3_medium/sa3_medium_essence_0.{5,7}.mp3`. **Validation workflow en attente de confirmation explicite** (règle AGENTS.md) — candidat : workflow « essence musicale » (`--audio` réf + `--text` style + échelle 0,5). Si validé → supprimer Stable-Audio-3-**Small** (2,2G, n'aurait plus d'usage).
