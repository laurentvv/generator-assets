...
* **Piste suivante (idée utilisateur, non validée) : « dubbing »/clonage vocal sur l'instrumental** — ① Dubbing parlé (`voix_off` sur l'instrumental, ducking sidechain) : testé (3 mixes v1/v2/v3) — jugé « pas terrible » (le TTS parle, ne chante pas ; v3 = bon équilibre niveaux). ② **SVC Seed-VC (`v1_svc`, F0)** : conversion du timbre du chant original vers la voix utilisateur — **TESTÉ, NON VALIDÉ** (POC vers 1 : `llb_chant_clone_v1.mp3`). Comportement par conception : **les paroles ET la langue originales sont conservées** (le clone chante l'anglais de la source — l'utilisateur « ne comprend rien »). ⚠️ Leverage d'API corrigé : `--audio` (chant source) + `--voice-ref` (timbre cible) + `--task-route v1_svc` (`--target-voice` est refusé par cette famille). Paquet `seed-vc-mlx-q8_0.gguf` (2,91 Gio) installé dans `C:\Modeles_LLM\SeedVC-MLX-GGUF`. **AUCUN workflow créé** (règle AGENTS : validation d'abord — un workflow `chant_clone` avait été créé par erreur sur un malentendu, immédiatement annulé, jamais commité). Cartographie vocale : cf. ci-dessus.
