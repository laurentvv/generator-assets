...
* **🧪 SA3 Medium gothic rock (2026-09-08, EN ATTENTE D'ÉCOUTE — instruction permanente, modèle signalé par la veille `hf-modeles-gguf`)** : `stable_audio_3_medium_f16` (5,43 Gio, `C:\Modeles_LLM\Stable-Audio-3-Medium-GGUF\`) — le « Medium » défaut de spec jamais testé (inventaire 2026-09-06 ; small-music jugé insuffisant en qualité). Recette de référence : tempo/tonalité **dans le prompt** (SA3 n'a pas d'options planner bpm/keyscale — ace_step only), graine 42, 30 s. **RTF 0,48 Vulkan** (30 s en 14,5 s — small = 0,25, ace_step turbo = 1,6, Music3 = 55,8), 44,1 kHz stéréo, brut −11,5 LUFS (+0,3 dBTP → limiteur avant tout usage). À écouter : `output/test_sa3_medium/sa3_medium_gothic_30s.mp3`. Contexte : essence SA3 small validée en mode (init_audio, échelle 0,7) mais qualité rejetée — Medium = le cran au-dessus, même moteur.
