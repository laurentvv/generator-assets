...
> La brique unitaire (1 chunk) est **validée** → workflow `h3_ref2va`
> (MEMORY_BANK §1.16). **La boucle elle-même n'a jamais été jugée** : les raccords
> chunk→chunk sont le point non validé. Ne deviendra un workflow qu'après
> validation utilisateur du résultat (règle AGENTS.md).

## 📏 Ce que ça produit (estimations mesurées, RX 6950 XT / 16 Go / 31,8 Go RAM)

| Fenêtre | Résultat attendu |
|---|---|
| **24 h** | ~18-20 chunks de 22 frames → **16-18 s de vidéo continue avec audio** (864×480, webm) |
| ~2 h 20 | 2 chunks → de quoi juger **le premier raccord** (recommandé comme 1re étape) |

Coût d'un chunk (~70 min) : encodage VAE de la référence ~14,5 min (CPU/swap) •
Qwen3-VL ~1,6 min • sampling 20 steps ~50 min (149 s/step, graph-cut) • décodage ~4 min.

**Leviers** (non validés) : steps 20→10 → ~27 s/24 h (qualité à tester) • résolution ÷2 →
~45-60 s/24 h • **RAM 64 Go = le vrai déblocage** (fin du swap VAE + DiT résident GPU
...
ls output/endless_dragon_24h/chunk_*.webm         # avancement (1 chunk ≈ 70 min)
```

**Critère go/no-go (1er raccord, ~2 h 20)** : extraire la dernière frame du chunk_01 et
la première du chunk_02 — si le dragon mute ou saute franchement, arrêter (inutile de
brûler 20 h sur une dérive). Micro-sauts possibles : Ref2VA en CLI n'a pas de boundary
