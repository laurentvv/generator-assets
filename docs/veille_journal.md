...
- **2026-09-06 • audio.cpp-gguf • ⚠️ MiniMax-Music3-GGUF a disparu des dépôts officiels** (HF `audio-cpp/audio.cpp-gguf` et miroir ModelScope vérifiés par API : plus aucun dossier ; les paquets de la spec n'ont plus de repo de téléchargement). **La copie locale `C:\Modeles_LLM\MiniMax-Music3-GGUF` est désormais irremplaçable** — la sauvegarder si le moteur alternatif §1.10 doit rester réinstallable. Même cas : les GGUF **Audio8_TTS/ASR** (nouveauté v0.7.2) n'ont pas de dossier public non plus (safetensors Audio8/… seulement). Par ailleurs inventaire complet fait le même jour : musique = ace_step/minimax_music3/**heartmula**/**midashenglm_gen**/stable_audio (dont **Medium**, défaut de spec, jamais testé) ; sep = htdemucs/bs_roformer/mel_band_roformer ; veille **sd-cli/stable-diffusion.cpp absent de `veille_versions.py`** (angle mort à combler).
