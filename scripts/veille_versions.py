...
def veille() -> tuple:
    ...
    # mise à jour passe par ses propres scripts (update.cmd + buildvulkan.cmd).
    # Repo SANS releases → surveillance au commit près, comparée au HEAD local.
    try:
        r = requests.get(
        ...
        try:
            ...
            if g.returncode == 0 and g.stdout.strip():
                local = g.stdout.strip()  # ex. « a69194f 2026-09-02 »
        except Exception:
            pass
        if local:
            ...
            if sha == sha_local:
                ...
                rapport.ajouter("🆕", "qwentts.cpp", f"nouveaux commits amont sur "
                                ...
                                f"OpenAI) — procédure : update.cmd (git pull) puis buildvulkan.cmd, "
                                f"smoke test qwen-tts avant/après ; ne jamais éditer les fichiers "
                                f"du clone à la main (doc dans le dépôt)")
        else:
