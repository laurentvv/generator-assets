...
def main() -> int:
    ...
    ap.add_argument("--cpu-threshold", type=float, default=60.0, help="seuil CPU %% (défaut 60)")
    ap.add_argument("--gpu-threshold", type=float, default=40.0, help="seuil GPU %% (défaut 40)")
    ap.add_argument("--ram-threshold", type=float, default=90.0, help="seuil RAM %% (défaut 90)")
    ap.add_argument("--vram-threshold", type=float, default=80.0, help="seuil VRAM %% (défaut 80)")
    ap.add_argument("--duration", type=float, default=3.0, help="durée d'échantillonnage en s (défaut 3)")
    args = ap.parse_args()
    ...
    print(f"🔍 Charge système ({len(data['samples'])} échantillons sur {args.duration:g} s) :")
    ...
    if vram_octets is not None:
        ...
        if data["vram_cap"] > 0:
            print(f"   VRAM dédiée : {vram_gio:.1f} / {data['vram_cap'] / 2**30:.0f} Gio")
    ...
    if exceeded:
        print(f"\n⛔ VERDICT : machine occupée ({', '.join(exceeded)}) — NE PAS lancer de génération maintenant.")
        return 1
