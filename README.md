...
* **Inputs**: `prompt` (English music description), `--moteur` (`acestep` défaut|`music3`), `--variante` (`turbo` défaut|`xl-turbo` 4B|`xl-sft` 4B+CFG, ACE-Step only), `--duration` (loop seconds, default 12), `--candidats` (default 3), `--lufs` (bed target, default −30), `--loop-mode` (`percussive`|`ambient`), `--music-backend` (`vulkan`|`cpu`), `--force-bpm`/`--tonalite`/`--mesure` (ACE-Step only), `--seed`, `--analyse` (Music Flamingo QA).
...
* **Example**:
  ```bash
  # Défaut = ACE-Step 1.5 (BPM libre, estimé en post-traitement) :
  ...
  # BPM/tonalité imposés au planner ACE-Step :
  ...
  # Variante XL 4B (DiT plus large, ~2,2x plus lent, paquets sur ModelScope :
  ...
  # Ancien moteur MiniMax-Music3 (~25 min/candidat) :
...
* **Licence**: MiniMax-Music3 community licence (MIT-style, commercial OK below $20M revenue; disclose AI-generated music in the video description: *« Musique : générée par IA (MiniMax-Music3) »*). ACE-Step 1.5 : **MIT** (aucune contrainte de mention).
