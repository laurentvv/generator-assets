...
* **Process**:
  1. Takes an existing image (`-i`) — ideally pre-matted, its alpha is preserved — or a **text prompt**: the source image is then generated by Flux.1 Vulkan and cleanly background-removed via the `generate` pipeline.
...
* **Example**:
  ...
  # From a plain prompt (image generated by Flux, then converted to 3D) + game-ready decimation :
